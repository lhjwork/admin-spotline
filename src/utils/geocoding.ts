...
// 한국 영역 내 좌표인지 확인
// 한국 좌표계 유효성 검사
export const isValidKoreanCoordinates = (lat: number, lng: number): boolean => {
  // 한국의 대략적인 경계
  const KOREA_BOUNDS = {
    north: 38.6,
    south: 33.0,
  ...
  return lat >= KOREA_BOUNDS.south && lat <= KOREA_BOUNDS.north && lng >= KOREA_BOUNDS.west && lng <= KOREA_BOUNDS.east;
};
...
export const extractAddressKeywords = (address: string): string[] => {
  const keywords = [];
...
export const validateCoordinateAccuracy = (coordinates: any): boolean => {
  ...
  for (let i = 0; i < coordinates.length - 1; i++) {
    for (let j = i + 1; j < coordinates.length; j++) {
      const distance = calculateDistance(coordinates[i].lat, coordinates[i].lng, coordinates[j].lat, coordinates[j].lng);
...
export const correctCoordinates = (coordinates: any): any => {
  ...
  const filtered = coordinates.filter((coord) => {
    const distances = coordinates.filter((c) => c !== coord).map((c) => calculateDistance(coord.lat, coord.lng, c.lat, c.lng));
...
// 좌표 포맷팅
export const formatCoordinates = (lat: number, lng: number, precision = 6): { lat: number; lng: number } => {
  return {
