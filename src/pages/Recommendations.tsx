...
function CreateRecommendationModal({ isOpen, onClose, onSuccess }) {
  ...
  const createMutation = useMutation(
    (data) => recommendationAPI.createRecommendation(data),
    {
      onSuccess: () => {
        onSuccess()
        onClose()
        ...
        <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              ...
              {errors.fromStore && (
                <p className="mt-1 text-sm text-red-600">{errors.fromStore.message}</p>
              )}
            </div>
            ...
            <div>
              ...
              >
                <option value="">매장 선택</option>
                {storesData?.filter(store => store._id !== fromStoreId).map(store => (
                  <option key={store._id} value={store._id}>
                    {store.name} ({store.category})
                  </option>
                ))}
              ...
              {errors.toStore && (
                <p className="mt-1 text-sm text-red-600">{errors.toStore.message}</p>
              )}
            </div>
...
export default function Recommendations() {
  ...
  const deleteMutation = useMutation(
    (id) => recommendationAPI.deleteRecommendation(id),
    {
      onSuccess: () => {
