...
import { TestTube, AlertTriangle, Settings, Play, Eye } from "lucide-react";

export default function DemoExperienceConfigs() {
  ...
  const loadConfigs = async () => {
    try {
      ...
      if (response.data.success) {
        // 데모용 설정만 필터링 (실제로는 백엔드에서 필터링해야 함)
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-lg">데모 체험 설정을 불러오는 중...</div>
      </div>
    );
  }
      ...
      <div className="flex justify-between items-start">
        <div>
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">데모 체험 설정</h1>
              <p className="text-gray-600 mt-1">데모용 "SpotLine 체험하기" 버튼의 동작을 설정합니다</p>
            </div>
          </div>
          ...
          <div className="mt-4 p-4 bg-orange-50 border border-orange-200 rounded-lg">
            <div className="flex items-start space-x-2">
              ...
              <div>
                <p className="text-sm font-medium text-orange-800">데모 전용 설정</p>
                <p className="text-sm text-orange-700 mt-1">이 설정들은 업주 데모 시연용으로만 사용되며, 실제 서비스에는 영향을 주지 않습니다.</p>
              </div>
            </div>
          </div>
        </div>

        <button onClick={() => setShowCreateModal(true)} className="flex items-center space-x-2 bg-orange-600 text-white px-4 py-2 rounded-lg hover:bg-orange-700">
          <Settings className="h-4 w-4" />
          <span>새 데모 설정 추가</span>
        </button>
      </div>
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-medium text-gray-900">데모 체험 설정 목록</h2>
            <div className="text-sm text-gray-500">총 {configs.length}개 설정</div>
          </div>
        </div>
            ...
            <div key={config._id} className="p-6">
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center space-x-2 ml-4">
                  <button
                    onClick={() => handlePreview(config._id)}
                    disabled={previewLoading[config._id]}
                    className="flex items-center space-x-1 text-blue-600 hover:text-blue-800 text-sm font-medium disabled:opacity-50"
                  >
                    <Eye className="h-4 w-4" />
                    <span>{previewLoading[config._id] ? "미리보기 중..." : "미리보기"}</span>
                  </button>

                  {!config.isDefault && (
                    <button onClick={() => handleSetAsDefault(config._id)} className="flex items-center space-x-1 text-green-600 hover:text-green-800 text-sm font-medium">
                  ...
                  <button
                    ...
                    className={`flex items-center space-x-1 text-sm font-medium ${config.isActive ? "text-red-600 hover:text-red-800" : "text-green-600 hover:text-green-800"}`}
                  >
                    <span>{config.isActive ? "비활성화" : "활성화"}</span>
                  </button>
                </div>
              </div>
              ...
              {previewResults[config._id] && (
                <div className="mt-4 p-4 bg-orange-50 rounded-lg">
                  <h4 className="text-sm font-medium text-orange-900 mb-2">데모 미리보기 결과 (5회 테스트)</h4>
                  <div className="space-y-2">
                    {previewResults[config._id].map((result, index) => (
                      <div key={index} className="text-sm text-orange-700 flex items-center space-x-2">
                        <span className="w-4 h-4 bg-orange-200 text-orange-800 rounded-full flex items-center justify-center text-xs">{index + 1}</span>
                        <span>{result.storeName}</span>
                        <span className="text-orange-500">({result.area})</span>
        ...
        {configs.length === 0 && <div className="p-6 text-center text-gray-500">등록된 데모 체험 설정이 없습니다.</div>}
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <div className="flex items-center space-x-2 mb-4">
              ...
              <h3 className="text-lg font-medium">새 데모 체험 설정 추가</h3>
            </div>
            <p className="text-gray-600 mb-4">데모 체험 설정 생성 기능은 곧 추가될 예정입니다.</p>
            <button onClick={() => setShowCreateModal(false)} className="w-full bg-orange-600 text-white py-2 rounded-lg hover:bg-orange-700">
              닫기
            </button>
          </div>
        </div>
