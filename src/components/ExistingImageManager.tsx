    ...
    <div className="space-y-6">
      ...
      <div>
        <h4 className="text-sm font-medium text-gray-900 mb-3 flex items-center">
          ...
          메인 배너 이미지 ({mainBannerImages.length}개)
        </h4>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {mainBannerImages.map((imageKey, index) => (
      ...
      {/* 삭제 확인 모달 */}
      {showDeleteModal && (
        <DeleteConfirmModal imageKey={showDeleteModal} />
      )}

      {/* 이미지 미리보기 모달 */}
      {previewImage && (
        <ImagePreviewModal imageUrl={previewImage} />
      )}
    </div>
  )
}