...
### 1.2 `src/components/curation/RouteSpotList.tsx` (수정)
...
4. RouteSpotList에 `distances` prop 전달
...
items 변경 → useMemo(calculateRouteDistances(items)) → distances 배열 생성
  → RouteSpotList에 distances 전달 (표시용)
...
| 2 | `src/components/curation/RouteSpotList.tsx` | dnd-kit 드래그 + 거리 표시 |
